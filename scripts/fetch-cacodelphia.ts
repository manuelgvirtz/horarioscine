...
async function main() {
  ...
  // ── Purge stale Cacodelphia showtimes for date >= today ──────────
  ...
  console.log(`\n${info} Limpiando horarios obsoletos de Cacodelphia (desde ${from})…`);
  await db.delete(showtimes).where(
    and(
      gte(showtimes.date, from),
      sql`${showtimes.cinemaId} IN (SELECT id FROM cinemas WHERE name ILIKE '%cacodelphia%')`,
    )
  );
