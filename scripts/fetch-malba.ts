...
async function main() {
  ...
  // ── Purge stale MALBA showtimes for date >= today ─────────────────
  ...
  console.log(`\n${info} Limpiando horarios obsoletos de MALBA (desde ${from})…`);
  await db.delete(showtimes).where(
    and(
      gte(showtimes.date, from),
      sql`${showtimes.cinemaId} IN (SELECT id FROM cinemas WHERE name ILIKE '%malba%')`,
    )
  );
